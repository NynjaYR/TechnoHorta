/*************************************************************
  ...
  Description : Subscribes to topic "pin_state"
                Turns ON/OFF pin 13 based on message
...
const char* ssid = "Сомнительный Wi-Fi";           // <-- Replace with your WiFi SSID
const char* password = "244466666";  // <-- Replace with your WiFi Password
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "pin_state") {
    if (incomingMessage == "rele_on_1" && currentState != "rele_on_1") {
      digitalWrite(ledPin, HIGH);
      Serial.println("Rele n_1 turned ON");
      currentState = "rele_on_1";
    }
    else if (incomingMessage == "rele_off_1" && currentState != "rele_off_1") {
